...
execute as @e[type=minecraft:armor_stand,limit=1,sort=nearest,tag=rav.rummban] if predicate ravrummagesack:itemban run scoreboard players set ban the_sack.drop 1
execute if data storage rav:rummagesack Item if entity @e[type=item,limit=1,sort=nearest,tag=rav.rummdrop] unless score ban the_sack.drop matches 1 unless data entity @s Inventory[{Slot:-106b}].tag."rav.sackcontents"[4] run function ravrummagesack:insertion/drop2
